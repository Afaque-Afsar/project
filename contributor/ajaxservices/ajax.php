<?php
	...
	if ( isset($_POST['type']) && $_POST['type'] == 'delete_discussion' ) {
		...
		$sql = "UPDATE discussion SET status = 2 WHERE added_by = ".$user_id." AND id = $discussion_id ";
        if ($con->query($sql)) {
        	$return = "true";
        } else {
			$return = "false";
		...
		$user_id = $_POST['user_id'];
		$sql = "UPDATE books SET status = 2 WHERE added_by = ".$user_id." AND id = $book_id ";
        if ($con->query($sql)) {
        	$return = "true";
        } else {
			$return = "false";
        ...
        $user_id = $_POST['user_id'];
        $sql = "UPDATE material SET status = 2 WHERE added_by = ".$user_id." AND id = $material_id ";
        if ($con->query($sql)) {
            $return = "true";
        } else {
            $return = "false";
        ...
        $user_id = $_POST['user_id'];
        $sql = "UPDATE past_papers SET status = 2 WHERE added_by = ".$user_id." AND id = $paper_id ";
        if ($con->query($sql)) {
            $return = "true";
        } else {
            $return = "false";
...
// 		$sql = "SELECT * FROM quiz WHERE category_id = $category_id AND status = 1 AND added_by = ".$_SESSION['user']['user_id']." ";
		...
		$user_id = $_POST['user_id'];
		$sql = "UPDATE questions SET status = 2 WHERE added_by = ".$user_id." AND id = $question_id ";
        if ($con->query($sql)) {
        	$return = "true";
        } else {
			$return = "false";
        ...
        $sql = "UPDATE questions SET quiz_part_id = $quiz_part_id, question = '$question', optionA = '$optionA', optionB = '$optionB', optionC = '$optionC', optionD = '$optionD',
                ...
                WHERE added_by = ".$user_id." AND id = $question_id ";
        if ($con->query($sql)) {
        	$return = "edited";
        } else {
	    ...
	    $user_id = $_SESSION['user']['user_id'];
        $sql = "UPDATE quiz_parts SET status = 2 WHERE id = ".$_POST['sub_quiz_id']." AND added_by = $user_id ";
        if( $con->query( $sql ) ){
